...
void interrupt MainInterrupt () {
    if (SSPIE && SSPIF) {
        ...
        unsigned char work      = 0x00;             // Temporary register
...
// BUTTON EDGE //
            case COMMAND_BUTT_EDGE :
                work  = (!!(address & 0x10));
                address             &= 0x0F;

                setButtonEdgeINT(address, work);
                break;

// BUTTON MODE //
            case COMMAND_BUTT_MODE:
                work  = (!!(address & 0x10));
                address             &= 0x0F;

                setButtonModeINT(address, work);
                break;
...
void clearInterruptData(void) {
    ...
    InterruptData.data14    = DUMMY_DATA;
    InterruptData.data15    = DUMMY_DATA;
    InterruptData.data16    = DUMMY_DATA;
}
...
void shiftInterruptData(void) {
    ...
    InterruptData.data13    = InterruptData.data14;
    InterruptData.data14    = InterruptData.data15;
    InterruptData.data15    = InterruptData.data16;
    InterruptData.data16    = DUMMY_DATA;
}
